...
import {
  TrendingUp, DollarSign, Users, Package, ShoppingCart, Target,
  RefreshCw, CheckCircle, BarChart3, ArrowUpRight, ArrowDownRight,
} from 'lucide-react';
...
interface Order {
  ...
  orderDate: string;
  totalPrice: number;
  orderDetails: Array<{ productId: number; productName?: string }>;
}
...
const EnhancedAnalyticsDashboard: React.FC = () => {
  ...
  useEffect(() => {
    async function fetchData() {
      ...
      try {
        ...
        const [products, orders, users, payments]: [
          Product[],
          Order[],
          User[],
          Payment[]
  ...
  const filteredData = useMemo(() => {
    ...
    if (filters.category !== 'all') products = products.filter(p => p.category?.categoryName === filters.category);
    if (filters.userType !== 'all') users = users.filter(u => u.userType === filters.userType);
    if (filters.paymentMethod !== 'all') payments = payments.filter(p => p.paymentMethod === filters.paymentMethod);
    if (filters.status !== 'all') orders = orders.filter(o => o.status === filters.status);
  ...
  const activeUsers = filteredData.users.filter(u => u.status === 'ACTIVE').length;
            ...
            <div className="flex items-center gap-3">
              <select
                ...
                onChange={(e: React.ChangeEvent<HTMLSelectElement>) =>
                  setSelectedTimeframe(e.target.value as '24h' | '7d' | '30d' | '90d' | '1y')
                }
      ...
      <main className="max-w-7xl mx-auto p-6">
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-6 mb-8">
          ...
          <MetricCard
            ...
            value={`${((filteredData.payments.filter(p => p.paymentStatus === 'COMPLETED').length / filteredData.payments.length) * 100).toFixed(1)}%`}
            icon={CheckCircle}
  ...
  <PieChart>
    ...
    <Tooltip />
        ...
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
          <ChartCard title="Order Status" icon={Target}>
          ...
          <ChartCard title="Payment Methods" icon={CheckCircle}>
            <ResponsiveContainer width="100%" height={250}>
              <PieChart>
                <Pie
